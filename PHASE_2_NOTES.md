...
Writing that last test surfaced a subtlety worth recording. The first version
...
### A test-assertion error worth recording
...
### GELU: tanh approximation, and the error is bounded by a test
...
### 1/√d attention scaling, with the reason measured
...
size when `v̂` is genuinely zero — which happens for real: an embedding row for a
...
### Gradient clipping is global, not per-tensor
